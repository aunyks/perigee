...
import { extname, posix } from 'https://deno.land/std@0.152.0/path/mod.ts';
...
import { Status, STATUS_TEXT } from 'https://deno.land/std@0.152.0/http/http_status.ts';
		...
		const body = new ReadableStream({
			...
			async pull(controller) {
				...
				controller.enqueue(
					bytes.slice(
						0,
						Math.min(bytesRead, contentLength - bytesSent),
					),
				);
				bytesSent += bytesRead;
...
function main(): void {
	...
	const handler = (req: Request): Promise<Response> => {
		// {
		// 	// This block is here to aid debugging WASM 
		// 	// in Chrome beta by loading Rust files that are 
		// 	// requested via the wasm file's extracted DWARF / sourcemap
...
INSTALL:
  deno install --allow-net --allow-read https://deno.land/https://deno.land/std@0.152.0/http/file_server.ts
